    ...
    *pFreeSpace = (SECTORNUM)0;
    ...
    if (state->snExtPartSector)
    {
        ...
        if (pPartState)
        {
            while (pPartState)
            {
                ...
                *pFreeSpace += snFreeSpace;
                ...
                if (snFreeSpace)
                {
                    if (state->diskInfo.nHeadsPerCylinder > 1)
                        snFreeSpace -= state->diskInfo.nSectorsPerTrack;
                    else
                        snFreeSpace--;
                }
            ...
            if (snFreeSpace)
            {
                if ((state->diskInfo.nHeadsPerCylinder > 1) && (snFreeSpace >= state->diskInfo.nSectorsPerTrack))
                    snFreeSpace -= state->diskInfo.nSectorsPerTrack;
                else
                    snFreeSpace--;
            }
        ...
        else
        {
            ...
            *pFreeSpace = state->snExtPartEndSec - state->snExtPartSector +1;
            ...
            if (state->diskInfo.nHeadsPerCylinder > 1)
                *pLargestPart = *pFreeSpace - state->diskInfo.nSectorsPerTrack;
            else
    ...
    if (tmpbuffer->Part_FirstTrack > 1)
    {
        if (state->diskInfo.nHeadsPerCylinder > 1)
        {
            ...
            snFreeSpace = tmpbuffer->Part_StartSector - state->diskInfo.nSectorsPerTrack;
            // adjust in case the partition doesn't start at head 0
            snFreeSpace -= tmpbuffer->Part_FirstHead * state->diskInfo.nSectorsPerTrack;
        }
        else
    ...
    for (i=0; i<4; i++, tmpbuffer++, tmpbuffer1++)
    {
        ...
        if (tmpbuffer->Part_TotalSectors && tmpbuffer1->Part_TotalSectors)
            ...
            snFreeSpace = state->diskInfo.nTotalSectors - (tmpbuffer->Part_StartSector + tmpbuffer->Part_TotalSectors);
...
// TODO: Verify that we support the LBA modes correctly
    ...
    dwBytesPerSector = state->diskInfo.nBytesPerSector;
...
BOOL CreatePBR(DriverState *state, SECTORNUM snLastPBRSec, SECTORNUM snNextPBRSec, SECTORNUM snPBRSector, SECTORNUM snNumSectors, BYTE fileSysType)
{
    ...
    if (state->diskInfo.nHeadsPerCylinder > 1)
        snDataStart = snPBRSector + state->diskInfo.nSectorsPerTrack;
    else
        snDataStart = snPBRSector + 1;

    buffer = (PBYTE)calloc(1,state->diskInfo.nBytesPerSector);//LocalAlloc(LMEM_ZEROINIT, state->diskInfo.nBytesPerSector);
    if (!buffer)
        return FALSE;
    ...
    *(WORD *)(buffer + BOOT_SIGNATURE) = BOOTSECTRAILSIGH;
    ...
    if (state->diskInfo.nHeadsPerCylinder > 1)
        tmpbuffer->Part_StartSector = state->diskInfo.nSectorsPerTrack;
    else
...
BOOL DeletePBR(DriverState *state, PartState *pPartState, PartState *pPrevPartState)
{
    ...
    if (pPrevPartState && pPrevPartState->snPBRSector)
    {
        ...
        memset(buffer, 0, state->diskInfo.nBytesPerSector);
...
BOOL WriteSectors(DriverState *state, SECTORNUM snSectorNum, SECTORNUM snNumSectors, PBYTE pBuffer)
{
	...
	drw.dwStartSector = (DWORD)snSectorNum;
	drw.dwSectorNumber = (DWORD)snNumSectors;
    drw.nrwNum = 1;
	drw.dwStatus = 0;
	drw.rwBufs[0].lpBuf = pBuffer;
	drw.rwBufs[0].dwSize = (DWORD)snNumSectors * state->diskInfo.nBytesPerSector;
    ...
    //sgReq.sr_sglist[0].sb_len = (DWORD)snNumSectors * state->diskInfo.nBytesPerSector;
    //bResult = DeviceIoControl(state->hStore, DISK_IOCTL_WRITE, &sgReq, sizeof(SG_REQ), NULL, 0, &dummy, NULL);
    bResult = DeviceIoControl(state->hStore, IOCTL_DISK_WRITE, &drw, sizeof(DISK_RW), NULL, 0, &dummy, NULL);
...
BOOL ReadSectors(DriverState *state, SECTORNUM snSectorNum, SECTORNUM snNumSectors, PBYTE *pBuffer)
{
    ...
    bufferSize = (DWORD)snNumSectors * state->diskInfo.nBytesPerSector;
    if (*pBuffer) {
        buffer = *pBuffer;
    } else {
    ...
    bResult = DeviceIoControl(state->hStore, IOCTL_DISK_READ, &drw, sizeof(DISK_RW), NULL, 0, &dummy, NULL);
...
BOOL GetPartLocation (DriverState * state, SECTORNUM numSectors, PartState **ppPartState, SECTORNUM *pStartSector, SECTORNUM *pPBRSector, SECTORNUM *pNumSectors)
{
    ...
    *pStartSector = 0;
    ...
    if (state->diskInfo.nHeadsPerCylinder > 1)
        if (numSectors < state->diskInfo.nSectorsPerTrack)
            *pNumSectors = state->diskInfo.nSectorsPerTrack;
        else
            if (numSectors % state->diskInfo.nSectorsPerTrack)
                *pNumSectors = (numSectors / state->diskInfo.nSectorsPerTrack +1) * state->diskInfo.nSectorsPerTrack;
    ...
    if (state->diskInfo.nHeadsPerCylinder > 1)
    {
        curNumSectors += state->diskInfo.nSectorsPerTrack;
        if (curNumSectors % (state->diskInfo.nSectorsPerTrack * state->diskInfo.nHeadsPerCylinder))
            curNumSectors = (curNumSectors / (state->diskInfo.nSectorsPerTrack * state->diskInfo.nHeadsPerCylinder)+1) * (state->diskInfo.nSectorsPerTrack * state->diskInfo.nHeadsPerCylinder);
    }
    else
        curNumSectors += 1;
    ...
    if ((!bLocFound) && snNextPartSector)
    {
        ...
        if (!bLocFound)
        {
            if (snNextPartSector == state->snExtPartSector)
            {
                ...
                if ((state->snExtPartEndSec - snNextPartSector) >= curNumSectors)
                {
                    bExtPart = TRUE;
                    bLocFound = TRUE;
    ...
    while (!bLocFound)
    {
        ...
        if (tmpbuffer->Part_FirstTrack > 1)
        {
            if (state->diskInfo.nHeadsPerCylinder > 1)
            {
                snGap = tmpbuffer->Part_StartSector - state->diskInfo.nSectorsPerTrack;

                // adjust in case the partition doesn't start at head 0
                snGap -= tmpbuffer->Part_FirstHead * state->diskInfo.nSectorsPerTrack;
                snNextPartSector = state->diskInfo.nSectorsPerTrack;
            }
            else
            {
        ...
        for (i=0; i<4; i++, tmpbuffer++, tmpbuffer1++)
        {
            ...
            if (!snGap)
            {
                if (tmpbuffer->Part_TotalSectors && tmpbuffer1->Part_TotalSectors)
                    ...
                    // calculate space from the last partition to the end of the drive
                    snGap = state->diskInfo.nTotalSectors - (tmpbuffer->Part_StartSector + tmpbuffer->Part_TotalSectors);
    ...
    *ppPartState = pLastPartState;
    ...
    if (bExtPart)
    {
        *pPBRSector = snNextPartSector;
        ...
        if (state->diskInfo.nHeadsPerCylinder > 1)
            *pStartSector = snNextPartSector + state->diskInfo.nSectorsPerTrack;
        else
...
BOOL ConvertLBAtoCHS (DriverState * state, SECTORNUM snSectorNum, PBYTE pCyl, PBYTE pHead, PBYTE pSector)
{
    ...
    if (sectorNum > state->diskInfo.nTotalSectors)
        return FALSE;

    sectorsPerCyl = state->diskInfo.nHeadsPerCylinder * state->diskInfo.nSectorsPerTrack;

    cyl = sectorNum / sectorsPerCyl;

    head = (sectorNum-(cyl * sectorsPerCyl)) / state->diskInfo.nSectorsPerTrack;

    sector = sectorNum - ((cyl * sectorsPerCyl) + (head * state->diskInfo.nSectorsPerTrack)) + 1;
...
BOOL WriteHiddenPartitionTable(DriverState * state)
{
    ...
    bufferSize = state->diskInfo.nBytesPerSector * HIDDEN_PART_SEC_COUNT;
    buffer = (PBYTE)calloc(bufferSize);//LocalAlloc(LMEM_ZEROINIT, bufferSize);
    if (!buffer)
        return FALSE;
...
//    bufferSize = ((4 * 1024 + state->diskInfo.nBytesPerSector - 1) / state->diskInfo.nBytesPerSector) * state->diskInfo.nBytesPerSector;
    bufferSize = state->diskInfo.nBytesPerSector;
    buffer     = (PBYTE)calloc(1,bufferSize);//LocalAlloc(0, bufferSize);
    if (buffer)
    {
        sectorNum  = pPartState->snStartSector;
        numSectors = (4*1024 + state->diskInfo.nBytesPerSector -1) / state->diskInfo.nBytesPerSector;
...
//        memset (buffer, 0xF6, bufferSize);
        ...
        for (;;)
        {
			...
			drw.dwSectorNumber = bufferSize / state->diskInfo.nBytesPerSector;
			drw.nrwNum = 1;
			drw.dwStatus = 0;
			drw.rwBufs[0].lpBuf = buffer;
			drw.rwBufs[0].dwSize = bufferSize;
            ...
            //sgReq.sr_num_sec          = bufferSize / state->diskInfo.nBytesPerSector;
            //sgReq.sr_num_sg           = 1;
            //sgReq.sr_status           = 0;
            //sgReq.sr_callback         = NULL;
            //sgReq.sr_sglist[0].sb_buf = buffer;
			...
			result = DeviceIoControl(state->hStore, IOCTL_DISK_WRITE, &drw, sizeof(DISK_RW), NULL, 0, &dummy, NULL);
            if (!result)
            {
                break;
            }

            sectorNum  += bufferSize / state->diskInfo.nBytesPerSector;
            numSectors -= bufferSize / state->diskInfo.nBytesPerSector;
        }
...
void GetPartition(DriverState * state, PartState ** ppPartState, PartState ** ppPrevPartState, LPCTSTR partName)
{
    ...
    *ppPartState = NULL;
    ...
    while (pPartState)
    {
		...
		if (stricmp(partName, pPartState->cPartName) == 0)
        {
...
DWORD GetDOSPartitions(DriverState * state, PartState ** pPrevPartState, PBYTE pSector, BOOL bExtPart, SECTORNUM *pStartSec)
{
    ...
    if (*(WORD *)(pSector + BOOT_SIGNATURE) == BOOTSECTRAILSIGH)
    {
        ...
        for (i=0, tmpbuffer = (PPARTENTRY)(pSector + PARTTABLE_OFFSET); i < partCount; i++, tmpbuffer ++)
        {
            if ((tmpbuffer->Part_BootInd != PART_NON_BOOTABLE) &&
                ...
                return bExtPart ? ERROR_SUCCESS : ERROR_INVALID_DATA;
    ...
    for (i = 0, tmpbuffer = (PPARTENTRY)(pSector + PARTTABLE_OFFSET); i < partCount; i++, tmpbuffer ++)
    {
        ...
        pPartState = (PPartState)calloc(1,sizeof(PartState));//LocalAlloc(LMEM_ZEROINIT, sizeof(PartState));
        if (!pPartState)
...
BOOL AddHiddenPartition(DriverState * state)
{
    ...
    pNewPartState = (PPartState)calloc(sizeof(PartState));//LocalAlloc(LMEM_ZEROINIT, sizeof(PartState));
    if (!pNewPartState)
        return FALSE;
...
void GeneratePartitionNames(DriverState *state)
{
    ...
    while (pPartState)
    {
                ...
                (*buffer == 0x90 || *buffer == 0xEB || *buffer == 0xE9))
            {
                ...
                if (memcmp( szVolumeName, "NO NAME", 7) != 0) {
                    ...
                    if (labellength) {
						...
						strncpy(pPartState->cPartName, szVolumeName, PARTITIONNAMESIZE );
                        pPartState = pPartState->pNextPartState;
                        free(buffer);
                        buffer = NULL;
                        continue;
        ...
        while (TRUE)
        {
            ...
            //wsprintf(partName, L"Part%02d",partID);
			sprintf(partName, "Part%02d",partID);
            partID++;
            ...
            if (!pTmpPartState)
            {
				...
				memcpy(pPartState->cPartName, partName, strlen(partName));
                break;
            }
        }
        pPartState = pPartState->pNextPartState;
