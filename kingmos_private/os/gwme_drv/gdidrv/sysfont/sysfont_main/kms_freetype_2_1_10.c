...
static int TTF_WordMask( HANDLE, const BYTE FAR* lpText, UINT len, _LPCHAR_METRICS );
...
typedef WORD (*LPGetCodeIndex)( HANDLE h, LPCBYTE lpbCode, UINT len );

extern WORD JapanCodec_GetCodeIndex( HANDLE h, LPCBYTE lpbCode, UINT len );
extern VOID * JapanCodec_Init();
...
#define _GET_CHINESE( lpfd, lpText, len )  ( (lpfd)->lpGetCodeIndex( (lpfd)->hCodecInst, (lpText), (len) )  )
...
static int    res = 72;
static   int  antialias = 1;//1;         /* is anti-aliasing active?    */
...
static FT_Error glyph_to_bitmap( 
					LPFONT_DATA lpfd,
					FT_Glyph    glyf,
                   grBitmap*   target,
                   int        *left,
...
static int GetMatchFace( const char*  filepath, const char * lpszFace, int indexStart, int * indexFind )
{
	int i;
	for( i = indexStart; i < num_fonts; i++ )
	{
		if( strcmp( fonts[i]->filepathname, filepath ) == 0 )
		{
			if( fonts[i]->family_name )
			{
				if( strcmp( fonts[i]->family_name, lpszFace ) == 0 )
				{	//找到
					*indexFind = i;
					return 1;
				}
...
static int install_font_file( const char*  filepath, DWORD dwCharset, int * firstFontIndex )
{
...
Success:
	...
	*firstFontIndex = num_fonts;
    ...
    for ( i = 0; i < num_faces; i++ )
    {
		...
		font->maxHeight = face->max_advance_height;


		strcpy( (char*)font->filepathname, filename );
		font->family_name = (char*)malloc( strlen( face->family_name ) + 1 );
		strcpy( (char*)font->family_name, face->family_name );

		font->face_index   = i;
		font->cmap_index   = face->charmap ? FT_Get_Charmap_Index( face->charmap ) : 0;
		font->dwCharSet = dwCharset;
...
static WORD GB2312_GetCodeIndex( HANDLE h, LPCBYTE lpbCode, UINT len )
{
	/*
	...
	*/
	extern UINT Gb18030ToUnicode(const UCHAR *gbstr, int * lpLen) ;

	//int len;
	return Gb18030ToUnicode( lpbCode, &len );

}

static WORD BIG5_GetCodeIndex( HANDLE h, LPCBYTE lpbCode, UINT len )
{
	/*
	...
	*/
	return QBig5Codec_convertToUnicode( lpbCode, len );

}


static WORD HANGUL_GetCodeIndex( HANDLE h, LPCBYTE lpbCode, UINT len )
{
	/*
	extern const unsigned short _949code[];
	...
	*/
	return QEucKrCodec_convertToUnicode( lpbCode, len );
}
...
static HANDLE TTF_CreateFont( DWORD dwData, CONST LOGFONT *lplf )
{
	...
	if( lpfd )
	{
		...
		if( lplf )
		{
			...
			switch( dwCharset = lplf->lfCharSet )
			{
			case GB2312_CHARSET:
//#ifdef LINUX_KERNEL
				lpszFontFileName = "./kingmos/system/CN-DFHEI5.TTF";
//#else
//				lpszFontFileName = ".\\kingmos\\sunfon.ttf";
//#endif
				lpfd->hCodecInst = (HANDLE)1;
				lpfd->lpGetCodeIndex = GB2312_GetCodeIndex;
				break;
			case CHINESEBIG5_CHARSET:
//#ifdef LINUX_KERNEL
				lpszFontFileName = "./kingmos/system/mingliu.ttc";
//#else
...
//#endif
			...
			case HANGUL_CHARSET:
//#ifdef LINUX_KERNEL
				lpszFontFileName = "./kingmos/system/gulim.ttc";
//#else
...
//#ifdef LINUX_KERNEL
				lpszFontFileName = "./kingmos/system/JP-FGCCASGM.TTC";
//#else
//				lpszFontFileName = ".\\kingmos\\JP-FGCCASGM.TTC";
//#endif
				lpfd->hCodecInst = (HANDLE)JapanCodec_Init();
				lpfd->lpGetCodeIndex = JapanCodec_GetCodeIndex;
				break;
			case ANSI_CHARSET:
				lpszFontFileName = "./kingmos/system/TT0009M.TTF";
				lpfd->hCodecInst = (HANDLE)1;
				lpfd->lpGetCodeIndex = GB2312_GetCodeIndex;
				break;
	...
	//		lpfd->lpGetCodeIndex = GB2312_GetCodeIndex;
		...
		if( lpszFontFileName )
		{
			...
			{	//没有 安装，安装...
				...
				_INTR_OFF();
#endif
				retv = install_font_file( lpszFontFileName, dwCharset, &index );
			...
			if( retv == 0 )
			{
				if( lplf )
				{
					...
					{
						int indexMatch;
						if( GetMatchFace( lpszFontFileName, lplf->lfFaceName, index, &indexMatch ) )
						{
							index = indexMatch;
						}
					}
				...
				//lpfd->yBaseline = (lpfd->font.height + 4)/ 5;
				
				{	
					PFont  pfont = (PFont)lpfd->font.face_id;
					lpfd->yBaseline = lpfd->font.height * ( -pfont->descender ) / pfont->maxHeight;
				}
...
{
	LPFONT_DATA lpfd = (LPFONT_DATA)handle;
    return lpfd->font.width / 2; //单字节宽度
}
...
static int TTF_WordMask( HANDLE handle, const BYTE FAR* lpText, UINT len, _LPCHAR_METRICS lpbmp )
{
    LPFONT_DATA lpfd = (LPFONT_DATA)handle;
   ...
   if( IS_CHINESE( lpText ) )
   {
	   ...
	   //if( lpbmp->top > n )
   	   ...
   	   lpbmp->bitmap.bmBitsPixel = ( grbmp.mode == gr_pixel_mode_gray ? 8 : 1 );//grbmp.grays;


	   //lpbmp->xAdvance -= (lpbmp->xAdvance - grbmp.width) /2;
	   //lpbmp->xAdvance = grbmp.width + lpbmp->left;
	   //lpbmp->xAdvance = lpfd->font.width;
...
//
	   ...
	   //lpbmp->xAdvance = lpfd->font.width / 2;
...
static int TTF_TextWidth( HANDLE handle, const BYTE FAR* lpText, int len )
{
   ...
   if( lpText )
   {
       while( !IS_TEXT_MARK( *lpText ) && *lpText && len )
       {
           if( IS_CHINESE( lpText ) )
           {
               w += TTF_WordWidth( handle, _GET_CHINESE( lpfd, lpText, len ) );
               lpText += 2;
               if( len > 0 )
                   len -= 2;
           }
...
static int TTF_TextHeight( HANDLE handle, const BYTE FAR* lpText, int aLineWidth )
{
    ...
    if( lpText )
    {
        ...
        do  {
            if( IS_CHINESE( lpText ) )
            {
				int cw = TTF_WordWidth( handle, _GET_CHINESE( lpfd, lpText, 2 ) );
                if( aLineWidth > 0 && 
					w + cw >= aLineWidth )
                {
                    h += fh;
