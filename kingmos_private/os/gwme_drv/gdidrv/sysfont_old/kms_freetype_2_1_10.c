...
init_freetype( void )
{
  ...
  static int
  install_font_file( const char*  filepath )
  {
...
static WORD HANGUL_GetCodeIndex( LPCBYTE lpbCode )
{
	...
	int offset;
	//949
	offset = (hiByte - 0x81) * 	178 + (loByte - 0x40) - ( loByte > 0x7e ); 
...
static HANDLE TTF_CreateFont( DWORD dwData, CONST LOGFONT *lplf )
{
	...
	if( lpfd )
	{
		...
		if( lpszFontFileName )
		{
			...
			if( retv == 0 )
			{
				lpfd->font.face_id = (FTC_FaceID)fonts[num_fonts - 1];
