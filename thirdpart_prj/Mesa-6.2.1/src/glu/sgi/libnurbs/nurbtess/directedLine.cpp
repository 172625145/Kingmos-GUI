...
#include <estdlib.h>
#include <estdio.h>
//#include <math.h>		// ÆÚ´ýÐÞ¸Ä Remarked By Liujun
#include "glimports.h"
#include "zlassert.h"
...
// Added By liujun to declare math functions
#include "gl\mlg_glstruct.h"
#include "gl\mlg_glmath.h"
#include "ealloc.h"
#include "myassert.h"
...
{
  FILE* fp = (FILE *)gl_fopen(filename, "w");
  assert(fp);
...
{
  Int i,j;
  FILE* fp = (FILE*)gl_fopen(filename, "r");
  assert(fp);
  Int nPolygons;
  gl_fscanf( fp, &nPolygons );
  directedLine *ret = NULL;
  ...
  for(i=0; i<nPolygons; i++)
    {
      ...
      gl_fscanf(fp, (int*)&(vert[0][0]));
      gl_fscanf(fp, (int*)&(vert[0][1]));
      gl_fscanf(fp, (int *)&(vert[1][0]));
      gl_fscanf(fp, (int*)&(vert[1][1]));
      VV[1][0] = vert[0][0];
      VV[1][1] = vert[0][1];
