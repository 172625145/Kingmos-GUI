...
Subdivider::join_s( Bin& left, Bin& right, Arc_ptr jarc1, Arc_ptr jarc2 )
{
	...
	Arc_ptr newright = new(arcpool) GL_Arc( arc_right, 0 ); 
	Arc_ptr newleft = new(arcpool) GL_Arc( arc_left, 0 );
	assert( t1 < t2 );
	if( isBezierArcType() ) {
...
Subdivider::join_t( Bin& bottom, Bin& top, Arc_ptr jarc1, Arc_ptr jarc2 )
{
	...
	Arc_ptr newtop = new(arcpool) GL_Arc( arc_top, 0 );
	Arc_ptr newbot = new(arcpool) GL_Arc( arc_bottom, 0 );
	assert( s1 > s2 );
	if( isBezierArcType() ) {
