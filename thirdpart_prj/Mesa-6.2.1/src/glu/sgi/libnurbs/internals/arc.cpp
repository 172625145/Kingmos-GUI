...
const int	GL_Arc::bezier_tag = (1<<13);
const int	GL_Arc::arc_tag = (1<<3);
const int	GL_Arc::tail_tag = (1<<6);
...
void
GL_Arc::makeSide( PwlArc *pwl, arc_side side )
{
    assert( pwl != 0);
    assert( pwlArc == 0 );
    assert( pwl->npts > 0 );
...
void
GL_Arc::getextrema( Arc_ptr extrema[4] )
{
