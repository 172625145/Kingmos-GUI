...
typedef class GL_Arc *Arc_ptr; 

enum arc_side { arc_none = 0, arc_right, arc_top, arc_left, arc_bottom };


class GL_Arc: public PooledObj { /* an arc, in two list, the trim list and bin */
...
public:
    ...
    inline		GL_Arc( GL_Arc *, PwlArc * );
    inline		GL_Arc( arc_side, long );
...
/*--------------------------------------------------------------------------
 * GL_Arc - initialize a new GL_Arc with the same type and uid of
 *	    a given GL_Arc and a given pwl arc
 *--------------------------------------------------------------------------
 */

inline
GL_Arc::GL_Arc( GL_Arc *j, PwlArc *p )
{
    bezierArc = NULL;
    pwlArc = p;
    type = j->type;
...
inline
GL_Arc::GL_Arc( arc_side side, long _nuid )
{
    bezierArc = NULL;
    pwlArc = NULL;
    type = 0;
