...
{ 
    ...
    Arc_ptr jarc = new(arcpool) GL_Arc( arc_bottom, 0 );
    arctessellator.bezier( jarc, smin, smax, tmin, tmin );
    ret->addarc( jarc  );
    pjarc = jarc->append( pjarc );

    jarc = new(arcpool) GL_Arc( arc_right, 0 );
    arctessellator.bezier( jarc, smax, smax, tmin, tmax );
    ret->addarc( jarc  );
    pjarc = jarc->append( pjarc );

    jarc = new(arcpool) GL_Arc( arc_top, 0 );
    arctessellator.bezier( jarc, smax, smin, tmax, tmax );
    ret->addarc( jarc  );
    pjarc = jarc->append( pjarc );

    jarc = new(arcpool) GL_Arc( arc_left, 0 );
...
Subdivider::Subdivider( Renderhints& r, Backend& b ) 
	: slicer( b ),
	  ...
	  arcpool( sizeof( GL_Arc), 1, "arcpool" ),
...
Subdivider::addArc( REAL *cpts, Quilt *quilt, long _nuid )
{
    ...
    GL_Arc *jarc  		= new(arcpool) GL_Arc( arc_none, _nuid );
    jarc->pwlArc	= 0;
