...
#include <estdio.h>
//#include <tchar.h>		// Erase By Liujun for a while
#include "wmesadef.h"
#include "GL/wmesa.h"
#include "mtypes.h"
#include "glapi.h"
...
//自己定义的系统函数，但是目前没有实现
//extern	DWORD	GetGlyphOutline( HDC hdc, UINT uChar, UINT uFormat,	LPGLYPHMETRICS lpgm,
    ...
    for( i = 0; i < MESAWGL_CTX_MAX_COUNT; i++ )
    {
        if ( wgl_ctx[i].ctx == NULL )
        {
            wgl_ctx[i].ctx = WMesaCreateContext( hWnd, NULL, GL_TRUE,
                ...
                (unsigned char)(pix[curPFD-1].pfd.cAlphaBits ? GL_TRUE : GL_FALSE));
            if (wgl_ctx[i].ctx == NULL)
                break;
            wgl_ctx[i].hdc = hdc;
            ctx_count++;
...
#define VERIFY(a) a
  ...
  for(i = 0; i < (int)numChars; i++) {
    ...
    curChar =(char)( i + firstChar);	// Modified By Liujun
    ...
    glBitmap(bmapWidth, bmapHeight, 
			 0.0, (GLfloat)metric.tmDescent,
                                       ...
                                       DWORD count, DWORD listBase)
{
   ...
   */
   for (i = 0; i < (int)count; i++)
   {
      ...
      (GLfloat)-gm.gmptGlyphOrigin.x, (GLfloat)gm.gmptGlyphOrigin.y,
       gm.gmCellIncX,gm.gmCellIncY, (const GLubyte * )lpBits);
