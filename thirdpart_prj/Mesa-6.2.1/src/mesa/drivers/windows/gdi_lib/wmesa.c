...
//extern	UINT SetDIBColorTable( HDC hdc, UINT uStartIndex, UINT cEntries, CONST RGBQUAD *pColors );
//extern	UINT GetNearestPaletteIndex( HPALETTE hpal, COLORREF crColor );
//extern	HPALETTE CreatePalette( CONST LOGPALETTE *lplgpl );
//extern	BOOL SetPixelV( HDC hdc, int X, int Y, COLORREF crColor );
//extern	HDC CreateIC( LPCTSTR lpszDriver, LPCTSTR lpszDevice, LPCTSTR lpszOutput, CONST DEVMODEA *lpdvmInit );
...
BOOL wmCreateBackingStore(PWMC pwc, long lxSize, long lySize)
{
  ...
  pwc->cColorBits = (BYTE)pbmi->bmiHeader.biBitCount;
