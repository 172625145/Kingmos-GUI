  ...
  Bool * allowDoubleAsSingle)
{
  ...
  cstr = strpbrk( (const char*)word, (const char*)"=><!~" );
  if (cstr) {
    switch (cstr[0]) {
    case '=':
      comparator = EQ;
  ...
  Criterion * requiredCriteria, int nRequired, int requiredMask)
{
  ...
  *allowDoubleAsSingle = False;
  ...
  if (!(mask & (1 << RGBA))) {
    ...
    criteria[n + 4].comparison = GL_min;
