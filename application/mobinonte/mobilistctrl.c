...
作者：jami chen 陈建明
修改记录：
  2004.06.15 修改显示，当指定宽度不能显示字串时，有原来
			 自己得到SHOR STRING改为有DrawText来完成

  2004.06.29 修改焦点条目的保存，使控制在失去焦点时保存原来
			 的焦点条目，当控件得到焦点时有可以设置到原来的
			 焦点条目上

  2004.07.09 增加消息功能WM_SETREDRAW , 用于在加入大量数据时是否
			 需要重新绘制窗口

  2004.07.19 增加消息功能WM_SYSCOLORCHANGE, 用于在在系统颜色重新
			 设置后，需要重新装载新的颜色

  2004.07.19 LVS_LIST风格的窗口也需要绘制水平线
...
#define	MAIN_SEL_ITEM_HEIGHT	39
#define MAIN_SEL_ITEM_WIDTH		400

const char classMOBILISTCTRL[14]="Mobi_ListView";
#define MOBI_LISTCTRL classMOBILISTCTRL
#define classMOBILISTVIEW classMOBILISTCTRL
...
// 外部变量引用区
extern	GLOBAL_MOBI	g_Mobi;
extern	HWND		hMainWnd;
extern	int			iMobiSel;
extern	DWORD		dszBgColor[6];
extern	BOOL		bTimerDraw;
// 全局变量定义区
int		Move_X = 100;
static	HDC	hMainTempDC = NULL;
...
//				BOOL bSort;      // Sort ?
				...
				int iActiveItem;
				int iFocusItem;
				int iOldFocusItem;  // 只有在窗口失去焦点和得到焦点是使用
				int iStartItem;
				int iPenCoordinate;
				int iNewWidth;
				int iDownRow;
...
//				LPSORTLISTITEM lpSortList;
				...
				BOOL bRedrawInChange; // add by Jami chen in 2004.07.09
};
...
/*********************************************************************************************************/
ATOM RegisterMobiListCtrlClass(HINSTANCE hInstance);
LRESULT CALLBACK MobiListCtrlProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
/*********************************************************************************************************/
...
// !!! delete By Jami chen in 2004.06.15
static int MakeShortString(HDC hdc, int nColumnLen,LPCTSTR lpLongString,LPTSTR lpShortString);
// !!! delete End By Jami chen in 2004.06.15
static int CalcStringWidth(HDC hdc, LPCTSTR lpString,int iMaxWidth);
...
static LRESULT DoSetRedraw(HWND hWnd,WPARAM wParam,LPARAM lParam);
...
ATOM RegisterMobiListCtrlClass(HINSTANCE hInstance)
{
	...
	wcex.lpfnWndProc	= (WNDPROC)MobiListCtrlProc;
	wcex.cbClsExtra		= 0;
	...
	wcex.lpszClassName	= MOBI_LISTCTRL;
...
引用: 
************************************************/
VOID DoDrawBkGnd( HWND hWnd, HDC hDc )
{
#if 0
		RECT rect;
		GetClientRect( hWnd, &rect );
		SelectObject( hDc, g_Mobi.cCommonMobi.hPen );
		SelectObject( hDc, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
		RoundRect( hDc, -5, rect.top, rect.right, rect.bottom, 10, 10 );	// 为了显示，所以用了-5，不让左边缘显示圆角
#endif
		if (!bTimerDraw)
		{
			...
			{
				RECT	rect;//, rtTemp;
				GetClientRect( hWnd, &rect );			// 获取列表框范围
//				SetRect( &rtTemp, 0-Move_X, 0, Move_X+rect.right, LIST_H );
//				if (rtTemp.left<rtTemp.right)
				if ( (rect.right+Move_X)>0 )	// 确保右边框显示
				{
					HDC		hTempDc;
					HBITMAP	hCompatibleBitmap, hTempBitmap;

					hTempDc = CreateCompatibleDC( hDc );	// 创建临时兼容DC

					hCompatibleBitmap = CreateCompatibleBitmap( hDc, rect.right, rect.bottom );		// 创建列表框大小的位图
					hTempBitmap = CreateCompatibleBitmap( hDc, rect.right, rect.bottom );			// 创建列表框大小的位图

					SelectObject( hCompatibleDC, hCompatibleBitmap );	// 选入兼容位图
					SelectObject( hMainTempDC, g_Mobi.hbitmapMobinonte[iMobiSel].hBigBkground );
					// hCompatibleDC 的可以显示的范围
					BitBlt( hCompatibleDC, 0-Move_X, rect.top, rect.right+Move_X, LIST_H, hMainTempDC, 0, LIST_Y, SRCCOPY );
//					BitBlt( hDc, rtTemp.left, rect.top, rtTemp.right, LIST_H, hMainTempDC, 0, LIST_Y, SRCCOPY );

					SelectObject( hTempDc, hTempBitmap );
					SelectObject( hTempDc, g_Mobi.cCommonMobi.hPen );
					SelectObject( hTempDc, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
//					FillRect( hTempDc, &rtTemp, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
//					RoundRect( hTempDc, rtTemp.left, rtTemp.top, rtTemp.right, rtTemp.bottom, 10,10 );
					RoundRect( hTempDc, 0, rect.top, rect.right, rect.bottom, 10,10 );
//					FillRect( hDc, &rtTemp, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );

					bAlpha.rgbTransparent = RGB(0,0,0);
					AlphaBlendEx( hCompatibleDC, rect.left, 0, rect.right-rect.left, LIST_H, hTempDc,
						rect.left, 0, rect.right-rect.left, LIST_H, &bAlpha );

					BitBlt( hDc, rect.left, 0, rect.right, LIST_H, hCompatibleDC, 0, 0, SRCCOPY );
		...
		else
		{
			...
			SelectObject( hDc, g_Mobi.cCommonMobi.hPen );
			SelectObject( hDc, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
			RoundRect( hDc, 0, rect.top, rect.right, rect.bottom, 10,10 );
		}
}
...
VOID	SetResourceDC( )
{
	...
	hMainTempDC = CreateCompatibleDC( hMainDC );

	ReleaseDC( hMainWnd, hMainDC );
}

VOID	ReleaseResourceDC( )
{
	if (hMainTempDC)
	{
		DeleteDC( hMainTempDC );
		hMainTempDC = NULL;
	}
}

extern	BOOL	bListHide;
LRESULT CALLBACK MobiListCtrlProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_GETMOVIEDC:
			{
#if 1
				DoDrawBkGnd( hWnd, (HDC)wParam );
				DoPaint( hWnd, (HDC)wParam );
#else
				HDC hDc = GetDC( hWnd );
...
//			break;
		case LVM_GETIMAGELIST : // 得到指定类型的图形列表句柄
			return DoGetImageList(hWnd ,wParam ,lParam);
...
//		case LVM_UPDATE :
//			break;
// Add By Jami chen in 2004.07.09
		case WM_SETREDRAW:
			return DoSetRedraw(hWnd,wParam,lParam);
// Add End By Jami chen in 2004.07.09

// Add By Jami chen in 2004.08.20
	case WM_SIZE: // 窗口大小发生改变
		return DoSize(hWnd,wParam,lParam);
	case WM_WINDOWPOSCHANGED:
		return DoWindowPosChanged(hWnd,wParam,lParam);
// Add End By Jami chen in 2004.08.20
		default:
		  {
				// default procedure
				LRESULT lResult;
...
static LRESULT DoCreateListCtrl(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
		LPPDALISTCTRL lpPDAListCtrl;
 			...
 			lpPDAListCtrl=(LPPDALISTCTRL)malloc(sizeof(PDALISTCTRL));  // 得到MOBI_LISTCTRL的数据结构
...
//			lpPDAListCtrl->cl_InvertBkColor = CL_BLUE;
			...
			lpPDAListCtrl->bRedrawInChange = TRUE;
			return 0;
}
/**************************************************
...
static void ReleaseListCtrl(HWND hWnd)
{
		...
		DWORD dwStyle;
			...
			lpPDAListCtrl=(LPPDALISTCTRL)GetWindowLong(hWnd,0); // 得到MOBI_LISTCTRL的数据结构
			if (lpPDAListCtrl==NULL)
					return ;
	  ...
	  {
		if (lpPDAListCtrl->lpImageList)
			ImageList_Destroy(lpPDAListCtrl->lpImageList);
		if (lpPDAListCtrl->lpImageListSmall)
...
static LRESULT DoLButtonDown(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
		...
		DWORD dwStyle;
		LPMAINITEM lpMainItem;
		BOOL bEditExist = FALSE;
//		POINT pt;
			lpPDAListCtrl=(LPPDALISTCTRL)GetWindowLong(hWnd,0); // 得到MOBI_LISTCTRL的数据结构
			if (lpPDAListCtrl==NULL)
					return 0;
			if (lpPDAListCtrl->hEdit)
			{
				bEditExist = TRUE;
			}
			if (GetFocus()!=hWnd) // 得到焦点
				SetFocus(hWnd); // 设置焦点
			if (bEditExist == TRUE)
					return 0;  // 如果原来存在编辑区，则处理完编辑区后将不再作任何处理
...
//			lpMainItem->y=0;
			...
			// Add By Jami chen in 2004.07.09
			if (lpPDAListCtrl->bRedrawInChange == TRUE)
			// Add End By Jami chen in 2004.07.09
			{ // 需要重新设置窗口
				SetVScrollBar(hWnd);
...
//				hIcon=PtrListAt(lpPDAListCtrl->lpImageListSmall,0);
				ImageList_GetIconSize(lpPDAListCtrl->lpImageListSmall,&(lpPDAListCtrl->sizeSmallIcon.cx),&(lpPDAListCtrl->sizeSmallIcon.cy));
...
//				ImageList_GetIconSize(hIcon,&(lpPDAListCtrl->sizeIcon));
				ImageList_GetIconSize(lpPDAListCtrl->lpImageList,&(lpPDAListCtrl->sizeSmallIcon.cx),&(lpPDAListCtrl->sizeSmallIcon.cy));
//				lpPDAListCtrl->sizeIcon.cx=32;
//				lpPDAListCtrl->sizeIcon.cy=32;
#endif
			}
...
static LRESULT DoSetFocus(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
	...
	LPMAINITEM lpMainItem;
			...
			lpPDAListCtrl->iFocusItem = lpPDAListCtrl->iOldFocusItem;
			if (lpPDAListCtrl->iFocusItem >= 0)
			{
				...
				{  // 设置当前条目得到焦点
					...
					pt.x=0;
					pt.y=0;
					SendNormalNotify(hWnd,LVN_ITEMCHANGED ,lpPDAListCtrl->iOldFocusItem,0,lpMainItem->State,uOldState,pt);
				}
				DrawItem(hWnd,lpPDAListCtrl->iFocusItem); // 绘制当前条目
			}
			SendBasicNotify(hWnd,NM_SETFOCUS);
			return 0;
}
...
static LRESULT DoKillFocus(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
	...
	DWORD dwStyle;
			...
			dwStyle=GetWindowLong(hWnd,GWL_STYLE); // 得到窗口的风格
			lpPDAListCtrl->iOldFocusItem = lpPDAListCtrl->iFocusItem;
...
static LRESULT DoEditLabel(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
	...
	int iImageWidth;
		...
		else
		{
			...
			rectItem.right = rectItem.left + strlen(lpMainItem->lpText) * lpPDAListCtrl->iTextWidth + 4;
			// !!! modified End
...
// !!! Add By Jami chen in 2004.08.12
			if (iOldFocus != -1)
			{
				// 原来存在hotitem,首先清除他
				ClearFocus(hWnd);
			}
// !!! Add End By Jami chen in 2004.08.12
			lpPDAListCtrl->iFocusItem=iIndex; // 设置指定索引为焦点
			lpPDAListCtrl->iOldFocusItem = lpPDAListCtrl->iFocusItem; // 设置旧的焦点条目

			if (lpPDAListCtrl->iOldFocusItem>= 0 && GetFocus() == hWnd)
			{
				// 当前窗口为焦点窗口
				LPMAINITEM lpMainItem;
...
//						lpMainItem->State=pItem->state&pItem->stateMask;
			...
			{ // 设置指定的条目
					...
					{ // 设置条目的状态
						lpMainItem->State &= ~pItem->stateMask; // clear Mask Bit
						lpMainItem->State |=pItem->state&pItem->stateMask;  // Set Mask Bit
...
// Add By Jami chen in 2004.07.09
/**************************************************
声明：static LRESULT DoSetRedraw(HWND hWnd,WPARAM wParam,LPARAM lParam)
参数：
	IN hWnd -- 窗口句柄
	IN wParam -- BOOL bRedrawInChange， 是否需要更新窗口
	IN lParam -- 保留 
返回值：无
功能描述：设置当窗口发生改变是否需要更新窗口，处理WM_GETCTLCOLOR消息。
引用: 
************************************************/
static LRESULT DoSetRedraw(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
	LPPDALISTCTRL lpPDAListCtrl;
		...
		lpPDAListCtrl=(LPPDALISTCTRL)GetWindowLong(hWnd,0); // 得到MOBI_LISTCTRL的数据结构
		ASSERT(lpPDAListCtrl);

		lpPDAListCtrl->bRedrawInChange = (BOOL)wParam;
...
static void DrawListCtrl(HWND hWnd,HDC hdc)
{
	...
	DWORD iDrawType;
		...
		dwStyle=GetWindowLong(hWnd,GWL_STYLE); // 得到窗口风格
		iDrawType=dwStyle&LVS_TYPEMASK; // 得到MOBI_LISTCTRL类型
		switch(iDrawType)
		{
...
static int GetLineHeight(HWND hWnd)
{
  ...
  DWORD dwStyle;
	...
	LPPDALISTCTRL lpPDAListCtrl;
		...
		ReleaseDC(hWnd,hdc);
// !!! Modify By Jami chen in 2004.09.18
/*
		nTextHeight = (int)TextMetric.tmHeight;

		nTextHeight += 2; // 得到文本高度
*/
		//nTextHeight = Sys_GetMetrics(SM_CYLISTITEM);		---> Modified By Liujun
		nTextHeight = MAIN_SEL_ITEM_HEIGHT;
// !!! Modify End By Jami chen in 2004.09.18
...
// !!! Delete By Jami chen in 2004.09.18
//		if (dwStyle&LVS_HORZLINE)
...
// !!! Delete End By Jami chen 2002.04.24
						...
						{ // 绘制文本
							...
							rect.bottom = rect.top +  lpPDAListCtrl->iTextHeight ;
							// !!! 
// !!! Modify By Jami chen in 2004.06.15
//							MakeShortString(hdc,rect.right-rect.left,lpSubItemString,lpDrawString);
//							iDrawStyle=DT_NOPREFIX|DT_SINGLELINE;
							iDrawStyle=DT_NOPREFIX|DT_SINGLELINE|DT_END_ELLIPSIS;
							lpDrawString = lpSubItemString;
// !!! Modify End By Jami chen in 2004.06.15
...
引用: 
************************************************/
static void ClearRectSpec(HWND hWnd,HDC hdc, int iDrawItem )
{
	RECT rect;
	int iRectHeight,iRectWidth;
	iRectHeight = iRectWidth = 0;
	rect.left = 0;
	rect.top = iDrawItem*MAIN_SEL_ITEM_HEIGHT+10;
	rect.right = MAIN_SEL_ITEM_WIDTH+28;
	rect.bottom = rect.top+MAIN_SEL_ITEM_HEIGHT;
	if (!bTimerDraw)
	{
		HDC	hCompatibleDC;
	//	if (rect.left<rect.right)
		if (rect.right+Move_X>0)
		{	// 用背景代替图片
	//		iRectWidth = rect.right - rect.left;		// 宽度
	//		iRectHeight = rect.bottom - rect.top;		// 高度
			iRectWidth = MAIN_SEL_ITEM_WIDTH+28;			// 宽度
			iRectHeight = MAIN_SEL_ITEM_HEIGHT;			// 高度
			...
			if (hCompatibleDC)
			{
				...
				hCompatibleBitmap = CreateCompatibleBitmap( hdc, iRectWidth, iRectHeight );		// 创建列表框大小的位图
				hTempBitmap = CreateCompatibleBitmap( hdc, iRectWidth, iRectHeight );			// 创建列表框大小的位图

				SelectObject( hCompatibleDC, hCompatibleBitmap );	// 选入兼容位图
				SelectObject( hMainTempDC, g_Mobi.hbitmapMobinonte[iMobiSel].hBigBkground );
				BitBlt( hCompatibleDC, 0-Move_X, 0, rect.right+Move_X, LIST_H, hMainTempDC, 0, LIST_Y+rect.top, SRCCOPY );

				{
					RECT rtTemp;
					//SetRect( &rtTemp, 0, 0, iRectWidth, iRectHeight );
					SetRect( &rtTemp, 0, 0, iRectWidth, iRectHeight );
					SelectObject( hTempDc, hTempBitmap );
					FillRect( hTempDc, &rtTemp, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
					DrawIcon( hTempDc, rect.right-45, 5, g_Mobi.cCommonMobi.hArrow );
	//				DrawIcon( hTempDc, 0, rect.top+5, g_Mobi.cCommonMobi.hArrow );
	//				BitBlt( hdc, rect.left, rect.top, iRectWidth, iRectHeight, hTempDc, 0,0, SRCCOPY );
				}

				bAlpha.rgbTransparent = -1;
				AlphaBlendEx( hCompatibleDC, 0, 0, iRectWidth, iRectHeight, hTempDc,
					0, 0, iRectWidth, iRectHeight, &bAlpha );
				BitBlt( hdc, 0, rect.top, iRectWidth, iRectHeight, hCompatibleDC, 0, 0, SRCCOPY );
				DeleteObject( hCompatibleBitmap );
				DeleteObject( hTempBitmap );
				DeleteDC( hTempDc );
				DeleteDC(hCompatibleDC);
	...
	{
		FillRect( hdc, &rect, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor ); // 清除矩形
		DrawIcon( hdc, rect.right-44, rect.top+5, g_Mobi.cCommonMobi.hArrow );
	}
}

static void ClearRect(HWND hWnd,HDC hdc,RECT rect)
{
#if 1		// Modified By Liujun
	return ;
#else
	 HBRUSH hBrush;
...
#endif
// !!! Delete End  By Jami chen in 2004.06.15
// !!! Add By Jami chen in 2004.06.15
/**************************************************
声明：static int CalcStringWidth(HDC hdc, LPCTSTR lpString,int iMaxWidth)
参数：
	IN hdc -- 设备句柄
	IN lpString -- 要计算的字符串
	IN iMaxWidth -- 字符串可以显示的最大长度
返回值：返回字符串的长度
功能描述：得到字符串的长度
引用: 
************************************************/
static int CalcStringWidth(HDC hdc, LPCTSTR lpString,int iMaxWidth)
{
	RECT rect;
		...
		rect.left = 0;
		rect.top = 0;
		rect.right = iMaxWidth+Move_X;
		rect.bottom = 32;

		// get text 's width and height in bottom and left of rectangle
//		DrawText( hdc, lpString, -1, &rect, DT_CALCRECT | DT_SINGLELINE | DT_END_ELLIPSIS);
		return rect.right - rect.left;  // 返回宽度
}
// !!! Add End  By Jami chen in 2004.06.15
...
//		TCHAR lpDrawString[128];
		...
		BOOL bDeleteBrush = TRUE;
			...
			if (lpPDAListCtrl==NULL)
					return ;
			iTopCoordinate=10;	// Liujun
			iLineHeight=GetLineHeight(hWnd); // 得到行高度

			dwStyle=GetWindowLong(hWnd,GWL_STYLE); // 得到窗口风格

// !!! modified by jami chen in 2004.06.29
/*
...
// !!! modified end
			...
			if (lpMainItem==NULL)
				return;
			SetPDAListColor(hWnd,hdc,NORMALCOLOR); // 设置颜色
// !!! modified by jami chen in 2004.07.19
//			ClearLine(hWnd,hdc,iDrawLine);
//			ClearRectSpec(hWnd,hdc,iDrawItem); // 清除条目
// !!! modified End by jami chen in 2004.07.19
			if (lpMainItem->State&LVIS_SELECTED)
			{ // 当前为选择条目
				int iSelL, iSelT, iSelR, iSelB, iSelWidth, iSelHeight;

				iSelL = MOBI_FORE_X;
				iSelT = iDrawItem*MAIN_SEL_ITEM_HEIGHT+10;
				iSelR = iSelL+MAIN_SEL_ITEM_WIDTH;
				iSelB = iSelT + MAIN_SEL_ITEM_HEIGHT;
				iSelWidth = MAIN_SEL_ITEM_WIDTH;
				iSelHeight = MAIN_SEL_ITEM_HEIGHT;
//--->Chenjianmin Code Begin
				SetPDAListColor(hWnd,hdc,INVERTCOLOR); // 设置颜色	// 
				bInvert=TRUE;
//---<Chenjianmin Code ENd
				if (bTimerDraw)
				{
#if 0
...
#endif
					RECT rt = { iSelL, iSelT, iSelR, iSelB };		// 注意范围
					SelectObject( hdc, g_Mobi.cCommonMobi.hPen );
					SelectObject( hdc, g_Mobi.hbitmapMobinonte[iMobiSel].hSelForegroundColor );
					FillRect( hdc, &rt, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
					RoundRect( hdc, iSelL, iSelT, iSelR, iSelB, 10, 10 );
					DrawIcon( hdc, iSelL+iSelWidth-45, iSelT+5, g_Mobi.cCommonMobi.hArrow );
				}
				else
				{	// 绘制前景
					HDC	hCompatibleDC;
//					if (iSelL<iSelR)
					if (MOBI_FORE_X+MAIN_SEL_ITEM_WIDTH+Move_X>0)
					{
						hCompatibleDC = CreateCompatibleDC( hdc );
						if( hCompatibleDC )
						{
							...
							SetRect( &rect, iSelL, iSelT, iSelR, iSelB );
							hTempDc = CreateCompatibleDC( hdc );	// 创建临时兼容DC

							hCompatibleBitmap = CreateCompatibleBitmap( hdc, iSelWidth, iSelHeight );	// 创建列表框大小的位图
							hTempBitmap = CreateCompatibleBitmap( hdc, iSelWidth, iSelHeight );			// 创建列表框大小的位图

							SelectObject( hCompatibleDC, hCompatibleBitmap );	// 选入兼容位图
							SelectObject( hMainTempDC, g_Mobi.hbitmapMobinonte[iMobiSel].hBigBkground );
							SelectObject( hMainTempDC, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
							BitBlt( hCompatibleDC, 0-Move_X-iSelL, 0, MOBI_FORE_X+MAIN_SEL_ITEM_WIDTH+Move_X, iSelHeight, hMainTempDC, 0, LIST_Y+iSelT, SRCCOPY );	// 注意范围

							SelectObject( hTempDc, hTempBitmap );
							SelectObject( hTempDc, g_Mobi.cCommonMobi.hPen );
							SelectObject( hTempDc, g_Mobi.hbitmapMobinonte[iMobiSel].hSelForegroundColor );
							{	
								RECT rt = { 0, 0, iSelWidth, iSelHeight };		// 注意范围
								FillRect( hTempDc, &rt, g_Mobi.hbitmapMobinonte[iMobiSel].hSelBackgroundColor );
							}
							RoundRect( hTempDc, 0, 0, iSelWidth, iSelHeight, 10, 10 );
							DrawIcon( hTempDc, iSelWidth-43, 5, g_Mobi.cCommonMobi.hArrow );

							bAlpha.rgbTransparent = -1;
							AlphaBlendEx( hCompatibleDC, 0, 0, iSelWidth, iSelHeight, hTempDc,
								0,0, iSelWidth, iSelHeight, &bAlpha );

							BitBlt( hdc, rect.left, rect.top, iSelWidth, iSelHeight, hCompatibleDC, 0, 0, SRCCOPY );
...
// !!! Add By Jami chen 2004.06.29
			if (!bInvert)
			ClearRectSpec(hWnd,hdc,iDrawItem); // 清除条目
// !!! add end By Jami chen 2004.06.29
...
//			iDrawStringWidth=MakeShortString(hdc,rect.right-rect.left,lpSubItemString,lpDrawString);
			iDrawStringWidth=CalcStringWidth(hdc, lpSubItemString,rect.right-rect.left);
			lpDrawString = lpSubItemString;
// !!! Modify End By Jami chen in 2004.06.15
...
//				FillRect(hdc,&rect,hBkBrush);
			...
			if (!bTimerDraw)
			{
				...
				SetRect( &rtTemp, rect.left, rect.top, rect.left+338+Move_X, rect.bottom );
//				if(rtTemp.left<rect.right)
				if(0<rect.right)
				{
					iOldMode = SetBkMode( hdc, TRANSPARENT );
					DrawText(hdc,lpDrawString,strlen(lpDrawString),&rtTemp,DT_LEFT|DT_SINGLELINE|DT_VCENTER);
...
//				dwRop=SRCINVERT;
			...
			hMemoryDC=hCompatibleDC; // 得到兼容DC
...
// !!! Modify End By Jami chen in 2004.06.15
			...
			// 绘制字串
			// Add End By Jami chen
			DrawText(hdc,lpDrawString,strlen(lpDrawString),&rect,DT_LEFT|DT_NOPREFIX|DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);
			bFocus=lpMainItem->State&LVIS_FOCUSED;
//			DrawFocus(hWnd,hdc,lpPDAListCtrl->iFocusItem,bFocus);
			if (bFocus)
...
//			DrawText(hdc,lpDrawString,strlen(lpDrawString),&rect,DT_CENTER|DT_NOPREFIX|DT_SINGLELINE|DT_VCENTER);
			DrawText(hdc,lpDrawString,strlen(lpDrawString),&rect,DT_CENTER|DT_NOPREFIX|DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);
			bFocus=lpMainItem->State&LVIS_FOCUSED;
			if (bFocus)
				DrawFocusRect(hdc,&rect); // 绘制焦点矩形
}
  ...
  SCROLLINFO ScrollInfo;
		...
		if (dwStyle&WS_VSCROLL)
		{
		  ...
		  // set vertical scroll page
			...
			SetScrollInfo(hWnd,SB_VERT,&ScrollInfo,FALSE); // 设置每页的大小
...
static	LRESULT CALLBACK LabelEditProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
static LRESULT DoEditKillFocus(HWND hWnd);
static LRESULT DoEditChar(HWND hWnd, WPARAM wParam, LPARAM lParam);
...
// !!! Add By Jami chen in 2004.08.24
//		SipShowIM(SIPF_ON);
// !!! Add End By Jami chen in 2004.08.24

		SetFocus(hEdit); // 设置焦点
		SendMessage(hEdit,EM_SETSEL,0,-1);  // 设置全部选择
}
/**************************************************
...
LRESULT CALLBACK LabelEditProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	LPPDALISTCTRL lpPDAListCtrl;
		...
		lpPDAListCtrl=(LPPDALISTCTRL)GetWindowLong(GetParent(hWnd),0); // 得到MOBI_LISTCTRL的数据结构
		if (lpPDAListCtrl==NULL)
			return 0;
#ifdef _MAPPOINTER
		...
		switch (message)
		{
		    ...
		    case WM_CHAR:
		       if (DoEditChar(hWnd,wParam,lParam) == TRUE) // 处理字符输入
				   return 0; // 字符已经处理
		}
...
static LRESULT DoEditKillFocus(HWND hWnd)
{
	...
	LPPDALISTCTRL lpPDAListCtrl;
		...
		lpPDAListCtrl=(LPPDALISTCTRL)GetWindowLong(hListView,0); // 得到MOBI_LISTCTRL的数据结构
		if (lpPDAListCtrl==NULL)
			return 0;
...
/**************************************************
声明：static LRESULT DoEditChar(HWND hWnd, WPARAM wParam, LPARAM lParam)
参数：
	IN hWnd -- 窗口句柄
返回值：无
功能描述：编辑窗口处理字符输入消息，WM_KILLFOCUS。
引用: 
************************************************/
static LRESULT DoEditChar(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
...
//		SipShowIM(SIPF_OFF);
// !!! Add End By Jami chen in 2004.08.24
		hParent = GetParent(hWnd); // 得到LISTVIEW 句柄
		...
		nmLVDispInfo.item=lvItem;
		// 得到条目信息
		SendMessage(hParent,WM_NOTIFY,(WPARAM)hWnd,(LPARAM)&nmLVDispInfo);
}
...
参数：
	IN hWnd -- 窗口句柄
	IN lpPDAListCtrl -- MOBI_LISTCTRL的数据结构
返回值：返回文本宽度,一个英文字符的宽度
功能描述：得到返回文本宽度,一个英文字符的宽度.
引用:  Add By Jami chen in 2004.05.21
************************************************/
