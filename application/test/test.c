...
//	wc.hIcon			= LoadImage(hInstance,MAKEINTRESOURCE(IDI_DEVICEMANAGE),IMAGE_ICON,16,16,0);
	...
	wc.style			= CS_HREDRAW | CS_VREDRAW;
	wc.lpfnWndProc		= (WNDPROC)TranWndProc; // 设备管理窗口过程函数
	wc.cbClsExtra		= 0;
	wc.cbWndExtra		= 0;
	wc.hInstance		= hInstance;
//	wc.hIcon			= LoadImage(hInstance,MAKEINTRESOURCE(IDI_DEVICEMANAGE),IMAGE_ICON,16,16,0);
	wc.hIcon			= NULL;
	wc.hCursor			= NULL;
	wc.hbrBackground	= NULL; // 设置背景刷
	wc.lpszMenuName		= NULL;
	wc.lpszClassName	= szWindowTranClass; // 设备管理窗口类名
	...
	wc.hIcon			= LoadImage(NULL,MAKEINTRESOURCE(OIC_APP_SYSTEM),IMAGE_ICON,16,16,0);// 装载应用程序图标
	wc.hCursor			= LoadCursor(NULL, IDC_ARROW);
	wc.hbrBackground	= CreateSolidBrush( RGB( 0, 0, 255 ) );//GetStockObject(GRAY_BRUSH); // 设置背景刷
	wc.lpszMenuName		= NULL;
	wc.lpszClassName	= szWindowRGNClass; // 设备管理窗口类名
...
static BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   // 创建窗口
   hWnd = CreateWindowEx(0,szWindowClass, "普通圆角窗口", WS_OVERLAPPEDWINDOW | WS_SYSMENU | WS_VISIBLE,
      30, 0, 210, 300, NULL, NULL, hInstance, NULL);

   // 创建窗口
   hWnd = CreateWindowEx(0,szWindowRGNClass, "特效窗口", WS_OVERLAPPEDWINDOW | WS_SYSMENU | WS_VISIBLE,
...
// ********************************************************************
//声明：static LRESULT CALLBACK TranWndProc(HWND , UINT , WPARAM , LPARAM )
//参数：
//	IN hWnd- 应用程序的窗口句柄
//  IN message - 过程消息
//  IN wParam  - 消息参数
